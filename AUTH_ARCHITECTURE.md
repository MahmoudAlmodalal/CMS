...
sequenceDiagram
    ...
    alt No User or role != 'admin'
        Route-->>Admin: 307 Redirect to /admin/login?redirect=/admin/artists
    ...
    Action-->>Admin: Redirect to /admin/login
```

### 3.1 Flow Breakdown

#### Flow 1: Admin Login
1. User navigates to [`/admin/login`](file:///home/mahmoud/Desktop/cms/src/app/admin/login/page.tsx).
...
3. If no active session exists on any `/admin/*` path (excluding `/admin/login`), middleware issues an immediate `307 Redirect` to `/admin/login?redirect=<target_path>`.
4. If session exists and user targets `/admin/login`, middleware redirects to `/admin`.
5. Request proceeds to [`src/app/admin/layout.tsx`](file:///home/mahmoud/Desktop/cms/src/app/admin/layout.tsx). Layout executes server-side validation asserting `user.app_metadata.role === 'admin'`.
...
4. If refresh token is revoked, expired, or invalid: `getUser()` returns `null`, middleware strips invalid cookies, and redirects user to `/admin/login?error=session_expired`.
...
6. Redirects browser to `/admin/login`.
...
flowchart LR
    ...
    Layer1 -.->|No Auth Cookie| RedirLogin["Redirect to /admin/login"]
...
- Inspects request pathname against pattern: `/admin/*` (excluding `/admin/login`, static assets, and favicon).
- Calls `supabase.auth.getUser()` to refresh session cookies.
- If unauthenticated: Redirects immediately to `/admin/login?redirect=${encodeURIComponent(pathname)}`.
- If authenticated visitor hits `/admin/login`: Redirects directly to `/admin`.
...
- If invalid: Calls `redirect('/admin/login')`.
...
| `/admin/login` | Unauthenticated Only | Middleware + Page | Normal render (shows login form) | Redirect to `/admin` |
| `/admin` | **Protected (Admin)** | Middleware + Layout | Redirect to `/admin/login?redirect=/admin` | Normal render (Dashboard) |
| `/admin/artists/**` | **Protected (Admin)** | Middleware + Layout + Action | Redirect to `/admin/login?redirect=/admin/artists` | Full CRUD |
| `/admin/events/**` | **Protected (Admin)** | Middleware + Layout + Action | Redirect to `/admin/login?redirect=/admin/events` | Full CRUD |
| `/admin/releases/**`| **Protected (Admin)** | Middleware + Layout + Action | Redirect to `/admin/login?redirect=/admin/releases`| Full CRUD |
| `/admin/news/**` | **Protected (Admin)** | Middleware + Layout + Action | Redirect to `/admin/login?redirect=/admin/news` | Full CRUD |
| `/admin/courses/**` | **Protected (Admin)** | Middleware + Layout + Action | Redirect to `/admin/login?redirect=/admin/courses` | Full CRUD |
| `/admin/testimonials/**`| **Protected (Admin)** | Middleware + Layout + Action | Redirect to `/admin/login?redirect=/admin/testimonials`| Full CRUD |
| `/admin/inquiries/**`| **Protected (Admin)** | Middleware + Layout + Action | Redirect to `/admin/login?redirect=/admin/inquiries` | View & Triage Leads |
| `/admin/settings` | **Protected (Admin)** | Middleware + Layout + Action | Redirect to `/admin/login?redirect=/admin/settings` | Edit Singleton Settings |
...
- **Resolution:**
  ...
  - If refresh fails (e.g., refresh token revoked or older than 7 days):
    - Clear auth cookies.
    - Redirect to `/admin/login?error=session_expired`.
...
- **Risk:** Incomplete cookie clearing causing infinite redirect between `/admin` and `/admin/login`.
- **Mitigation:**
  - Middleware explicitly tests if request pathname is exactly `/admin/login`.
  - If `getUser()` returns valid admin on `/admin/login`, redirect to `/admin`.
  - If `getUser()` returns null or non-admin on `/admin/*` (not `/admin/login`), redirect to `/admin/login`.
  - Prevents recursive rewrite loops.

### 7.4 Open Redirect Defense on `redirect` Parameter
- **Threat:** Attacker crafts phishing link: `/admin/login?redirect=https://evil-site.com`.
