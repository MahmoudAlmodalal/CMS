-- Custom work thumbnails (artist_works.thumbnail_image_url, stored in artists/gallery)
-- are storage references too: without them here, cleanup would delete thumbnails
-- still in use. Mirrors MEDIA_REFERENCES in src/lib/storage.ts.
-- Depends on: 20260918000000 (artist_works).
-- Idempotent: CREATE OR REPLACE with the same output columns as 20260917000000.
...
WITH active_db_urls AS (
  ...
  UNION ALL
  SELECT thumbnail_image_url FROM public.artist_works WHERE thumbnail_image_url IS NOT NULL
)
SELECT
  o.id,
  o.bucket_id,
